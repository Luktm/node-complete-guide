...
module.exports.getProducts = (req, res, next) => {
    ...
    // Sequaliz model to connect and retrieve db
    // Product
    //     .findAll()

    // mongodb Product model invoke here
    Product
        .fetchAll()
        .then((products) => {
...
module.exports.postCart = (req, res, next) => {
    ...
    Product.findById(prodId)
        .then((product) => {
        // so we can leveraging User model function we set.
        // addToCart is promises, so we can put then at line 144 to get the result
        return req.user.addToCart(product); // ? <- req.user has contain id already cuz req.user assigned from app.js line 81 with user id.
    }).then((result) => {
        console.log(result);
        res.redirect('/cart');
    });
    ...
    // let fetchedCart;
    // let newQuantity = 1;
    // req.user.getCart has relation between User and Cart, that's why we can use getCart invocation
    // req.user been assign new User() model, so it can access user model, app.js line number 81.
...
module.exports.postCreateOrder = (req, res, next) => {
    ...
    // mongodb here
    // remember User() was given in app.js line 81, so it did bind with user id
    req.user
        .addOrder()
        .then((result) => {
            console.log(result);
